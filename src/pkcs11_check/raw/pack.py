...
class PackedMechanism:
    ...
    it is None. Named buffers expose mutable output buffers after a raw call.
    """
    ...
    def add_buffer(self, name: str, storage: Any, length: int) -> None:
        """Keep a mutable output buffer alive and expose it by name."""
        if length < 0:
            raise ValueError("buffer length must be non-negative")
        self._keepalive.append(storage)
        self._named_buffers[name] = (storage, length)
...
__all__ = [
    ...
    "template_from_dict",
    "template_ptr_count",
    # Re-exported from pack_mechanisms for the public surface area:
    "mech_cbc_pad",
    "mech_ccm",
    "mech_ccm_message_generated_nonce",
    "mech_ccm_wrap",
