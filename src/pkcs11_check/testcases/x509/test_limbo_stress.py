...
@pytest.mark.parametrize("tc_id,der_bytes", _all_certs[:500], ids=lambda x: f"cert-{x}" if isinstance(x, str) else "cert")
def test_exhaustive_cert_import_no_crash(tc_id: str, der_bytes: bytes, p11_session: Any, limbo_available: Any) -> None:
...
@pytest.mark.parametrize("tc_id,der_bytes", _all_crls, ids=lambda x: f"crl-{x}" if isinstance(x, str) else "crl")
def test_exhaustive_crl_import_no_crash(tc_id: str, der_bytes: bytes, p11_session: Any, limbo_available: Any) -> None:
