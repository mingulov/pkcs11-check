...
class TestCertificateAttributes:
    ...
    def test_verify_attributes(
        ...
        try:
            ...
            try:
                ...
                if not subject:
                    note(
                        f"Module returned empty CKA_SUBJECT for {tc['id']}", ComplianceLevel.WARNING
                    )
            except (PKCS11Error, KeyError):
            ...
            try:
                ...
                if not issuer:
                    ...
                    note(
                        f"Module returned empty CKA_ISSUER for {tc['id']}", ComplianceLevel.WARNING
                    )
            except (PKCS11Error, KeyError):
                pass
            ...
            try:
                ...
                if not serial:
                    ...
                    note(
                        f"Module returned empty CKA_SERIAL_NUMBER for {tc['id']}",
    ...
    def test_import_with_trusted_flag(
        self, p11_session: Any, limbo_available: Any, p11_interface_version: str
    ) -> None:
