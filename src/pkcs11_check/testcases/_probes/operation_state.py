...
``CROSS_SESSION_REJECTED:0x...``, ``CKR:...``, ``SETUP_XFAIL:...``, and ``OK:...``)
are parsed by the parent after the child has completed, so provider responses remain
distinct from harness/process failures.
...
def _run_digest_same_session(ctx: ProbeContext, _extra: dict[str, Any]) -> None:
    ...
    if rv in (CKR_STATE_UNSAVEABLE, CKR_FUNCTION_NOT_SUPPORTED):
        print(f"CKR:GetState_len:0x{rv:08x}")
        return
    if rv != CKR_OK:
        print(f"CKR:GetState_len:0x{rv:08x}")
        return
    ...
    if rv in (CKR_STATE_UNSAVEABLE, CKR_FUNCTION_NOT_SUPPORTED):
        print(f"CKR:GetState_data:0x{rv:08x}")
        return
    if rv != CKR_OK:
        print(f"CKR:GetState_data:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:SetOperationState:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:DigestUpdate_part2:0x{rv:08x}")
        return
    dlen2 = c_ulong(32)
    dbuf2 = (c_ubyte * 32)()
    rv = raw.C_DigestFinal(sh, dbuf2, byref(dlen2))
    if rv != CKR_OK:
        print(f"CKR:DigestFinal_mp:0x{rv:08x}")
        return
    restored = binascii.hexlify(bytes(dbuf2[: dlen2.value])).decode()
    print(f"RESTORED:{restored}")
    print("OK:digest_same_session")
...
def _run_digest_cross_session(ctx: ProbeContext, _extra: dict[str, Any]) -> None:
    ...
    part1 = _CROSS_SESSION_PART1
    part2 = _CROSS_SESSION_PART2
    part1_buf = _byte_array(part1)
    print(f"REFERENCE:{hashlib.sha256(part1 + part2).hexdigest()}")

    rv = raw.C_DigestInit(sh, ctypes.byref(mech))
    if rv != CKR_OK:
        print(f"CKR:DigestInit:0x{rv:08x}")
        return

    rv = raw.C_DigestUpdate(sh, part1_buf, c_ulong(len(part1)))
    if rv != CKR_OK:
        print(f"CKR:DigestUpdate:0x{rv:08x}")
        return
    ...
    try:
        ...
        print(f"SETUP_XFAIL:OpenSession2 failed: {exc}")
        return
    ...
    if rv2 == CKR_OK:
        ...
        part2_buf = _byte_array(part2)
        rv3 = raw.C_DigestUpdate(sh2, part2_buf, c_ulong(len(part2)))
        if rv3 != CKR_OK:
            print(f"CKR:DigestUpdate_cross:0x{rv3:08x}")
            close_session_quietly(raw, sh2)
            return
        dlen = c_ulong(32)
        dbuf = (c_ubyte * 32)()
        rv3 = raw.C_DigestFinal(sh2, dbuf, byref(dlen))
        if rv3 != CKR_OK:
            print(f"CKR:DigestFinal_cross:0x{rv3:08x}")
            close_session_quietly(raw, sh2)
            return
        restored = binascii.hexlify(bytes(dbuf[: dlen.value])).decode()
        print(f"RESTORED:{restored}")
    else:
        print(f"CROSS_SESSION_REJECTED:0x{rv2:08x}")

    close_session_quietly(raw, sh2)
    print("OK:digest_cross_session")
...
def _run_encrypt_same_session(ctx: ProbeContext, _extra: dict[str, Any]) -> None:
    ...
    if rv != CKR_OK:
        print(f"CKR:EncryptUpdate_ref1:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:EncryptUpdate_ref2:0x{rv:08x}")
        return
    ...
    rv = raw.C_EncryptInit(sh, ctypes.byref(enc_mech), h_key)
    if rv != CKR_OK:
        print(f"CKR:EncryptInit_mp:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:EncryptUpdate_mp1:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:EncryptUpdate_mp2:0x{rv:08x}")
        return
    ...
    if rv != CKR_OK:
        print(f"CKR:EncryptFinal_mp:0x{rv:08x}")
        return
