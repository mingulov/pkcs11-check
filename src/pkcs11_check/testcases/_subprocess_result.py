...
def assert_subprocess_completed(
    ...
    *,
    ...
    if rc >= 0 and (harness_error := _harness_error_line(stdout, stderr)) is not None:
        # OUR code broke, not the module's (GH #9/#11). Checked before the exit-code
        # branches so a harness bug is never published as a provider crash finding --
        # but only for rc >= 0: a signal means the module took the process down, and
        # that stays a module finding whatever we had already printed.
