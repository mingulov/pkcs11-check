...
def _license_files_from_entry(entry: dict[str, object]) -> list[str]:
    """Return the license_files paths declared for a manifest entry."""
    raw = entry.get("license_files")
    if not isinstance(raw, list):
        return []
    return [str(p) for p in raw]


def _print_license_line(entry: dict[str, object]) -> None:
    """Print the SPDX + upstream-URL line for a manifest entry."""
    license_id = str(entry.get("license") or "unknown")
    paths = _license_files_from_entry(entry)
    if not paths:
        console.print(
            f"  license: {license_id} "
            "[dim](no upstream license file pinned — see THIRD_PARTY_LICENSES.md)[/dim]"
        )
        return
    repo = str(entry["repo"])
    commit = str(entry["commit"])
    url = f"https://github.com/{repo}/blob/{commit}/{paths[0]}"
    console.print(f"  license: {license_id} — {url}")


def _locate_upstream_license_paths(dest: Path, license_files: list[str]) -> list[Path]:
    """Resolve declared license paths on disk after extraction.

    For each declared path `p`, try `dest/p` first; if missing, look one
    level down (GitHub archives extract as `dest/<repo>-<sha>/...`). Returns
    the resolved paths in declaration order; entries that cannot be found
    are simply omitted.
    """
    found: list[Path] = []
    if not dest.is_dir():
        return found
    top_subdirs = [child for child in dest.iterdir() if child.is_dir()]
...
def _fetch_one(name: str, entry: dict[str, object], data_dir: Path) -> bool:
    ...
    license_files = _license_files_from_entry(entry)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for path in _locate_upstream_license_paths(dest, license_files):
            console.print(f"  upstream license: {path}")
    ...
    for name, entry in sources.items():
        ...
        if dest.exists():
            ...
            console.print(
                ...
                f"[dim][{license_id}][/dim] "
                f"[dim](pkcs11-check fetch-data {name})[/dim]"
            )
    console.print()
    console.print(
        "[dim]Each archive carries its own license \u2014 see THIRD_PARTY_LICENSES.md.[/dim]"
    )
...
def fetch_data_command(
    ...
    console.print(
        "[dim]Each fetched archive carries its own license — see THIRD_PARTY_LICENSES.md.[/dim]"
    )
