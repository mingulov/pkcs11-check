...
- **render** (`render.py`) - emits the compact provider markdown: a counts line, crash and fail
  ...
  one-liners for sanctioned-refusal compliance and the migration backlog; unclassified finding
  details remain in the severity-ranked fail sections rather than being folded into that count.
  The migration-backlog count stays one line, keeping the report near one screen even at
  thousands of findings.
- **correlate** (`correlate.py`) - `enrich()` annotates each group in place with a triage
  `category` / `routing` (provider fails, including unclassified fail-closed evidence →
  `PROVIDER_BUG`/`PROVIDER_REPORT`, xfails → reason-specific deviation routes, harness errors →
...
| field | meaning |
|---|---|
| `reason` | one of the 10 reasons (`wrong_result`, `accepted_invalid`, `self_contradiction`, `oracle`, `crash`, `not_operational`, `nonspec_reject`, `honest_deviation`, `undeclared_capability`, `sanctioned_refusal`) plus the reserved `unclassified` runtime-gate marker; `unclassified` is provider-fail evidence and a separately counted migration backlog |
