...
| Wycheproof ECDSA/DSA deduplication | Valid. PKCS#11 receives raw `r || s`, so DER/P1363 rows that normalize to the same public key, digest/message, signature, mechanism, and result are duplicate provider inputs. | Invalid vectors whose only invalidity was DER container metadata must be described as loader-normalized duplicates, not provider passes. |
