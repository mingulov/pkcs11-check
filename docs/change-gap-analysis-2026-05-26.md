...
| EdDSA runtime classification | Valid for advertised-but-not-operational paths. Unsupported curves remain skips; generic runtime import/sign/verify rejects become xfail evidence. | Valid-vector runtime rejects are still provider findings, so article wording should separate "unsupported" from "advertised but failing." |
...
| Wycheproof ECDSA/DSA deduplication | Valid. PKCS#11 receives raw `r || s`, so DER/P1363 rows that normalize to the same public key, digest/message, signature, mechanism, and result are duplicate provider inputs. ECDSA fixed-width P1363 `SignatureSize` negatives are also not provider-neutral when the raw signature is a shorter even-length `r || s` form allowed by the current PKCS#11 spec. DSA public-key imports now also strip Wycheproof's ASN.1-style sign padding from `Big integer` attributes before calling `C_CreateObject`. | Invalid vectors whose only invalidity was DER container metadata, Bitcoin low-S policy, fixed-width P1363 size policy, or third-party integer container padding must be described as loader-normalized or version-sensitive skips, not provider passes. |
| Wycheproof RSA/RSA-PSS deduplication | Valid. Fingerprints include mechanism, mechanism parameters where applicable, public key, message, and signature. | Need refreshed Docker rows because this patch landed after several targets had already started. |
