...
| A12 | `wycheproof/test_wycheproof_rsa_decrypt.py:152` (skip at :152) | `Cannot import RSA {bits}-bit private key: …` | raw `import_rsa_private_key` | **NONE — row error (verified 2026-06-11): no `has_mechanism("RSA_PKCS")` gate exists; the :152 skip is an unconditional catch-all (the broad-CKR check only gates the size cache)** | **needs gating + wiring** → add `has_mechanism("RSA_PKCS")` gate, then `import_rsa_private_key_negotiated`, then skip→xfail (left out of 45441f10 deliberately) |
