...
Failures **151** · file-crashes **1** · classes **56** (sum 151). Root causes: [catalog.md](catalog.md); raw: [failure-inventory.json](failure-inventory.json).
...
- **[14]** `test_mldsa_sign` — Invalid ML-DSA sign vector VEC accepted by module
- **[9]** `TestMlDsaSigVer::test_acvp_mldsa_sigver` — ML-DSA-sigVer-ML-DSA-N-tc: module rejected a VALID ML-DSA signature
- **[8]** `test_mldsa_verify` — Invalid ML-DSA sig VEC accepted by module
...
- **[1]** `TestRSAPaddingOracle::test_oaep_error_uniformity` — SECURITY: RSA-OAEP padding oracle — non-uniform error codes: {'CKR_ENCRYPTED_DATA_INVALID', 'CKR_ARGUMENTS_BAD
