...
3. Dedicated DSA/DH/X9.42 domain-parameter coverage exists in
   ...
   also reject signatures with an extra trailing byte. DSA prehash multipart
   sign/verify coverage now verifies every generated-key DSA-with-hash variant
   can sign and verify chunked input through `C_SignUpdate`/`C_SignFinal` and
   `C_VerifyUpdate`/`C_VerifyFinal`. Classic DH
   ...
   typed parameters and matching derived outputs.
   X9.42 hybrid/MQV OtherInfo negative coverage now verifies both extended
...
7. SHAKE/XOF dedicated coverage exists: raw `C_DigestXof*` function
   ...
   missing. KMAC variable-output-length coverage now runs nonzero short
   `ulMacLength` sign/verify roundtrips for both KMAC mechanisms. KMAC
   tampered-message negative coverage now verifies that a valid KMAC signature
   over the original data does not verify after the data is modified.
   Remaining adjacent work is deeper KMAC provider evidence and deeper ML-DSA
   ExternalMuGen/PQC provider evidence, not basic SHAKE/XOF raw API coverage.
...
10. Generic negative coverage is improving but still incomplete relative to
    ...
    keys, with metadata self-contradictions classified as failures and honest
