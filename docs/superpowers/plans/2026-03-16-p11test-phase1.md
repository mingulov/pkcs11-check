# p11test Phase 1 Implementation Plan
...
**Architecture:** Single Python package (`p11test`) with src/ layout. Core engine handles module loading, subprocess-based test isolation, and three-layer config. Test cases are native pytest tests. CLI built on typer+rich. python-pkcs11 (upstream) for v2.40; v3.x fork deferred to Phase 1b.
...
src/p11test/__init__.py                # version, package metadata
src/p11test/config.py                  # P11TestConfig (pydantic-settings)
src/p11test/plugin.py                  # pytest11 entry point
src/p11test/fixtures.py                # pytest fixtures (p11_module, p11_session, etc.)
src/p11test/cli/__init__.py
src/p11test/cli/app.py                 # typer app, main() entry point
src/p11test/cli/test_cmd.py            # `p11test test` command
src/p11test/cli/info_cmd.py            # `p11test info` command
src/p11test/cli/list_cmd.py            # `p11test list` command
src/p11test/core/__init__.py
src/p11test/core/loader.py             # PKCS#11 module loading + interface negotiation
src/p11test/core/isolation.py          # subprocess test runner, TestResult
src/p11test/core/timeout.py            # timeout management
src/p11test/core/session.py            # PKCS#11 session lifecycle
src/p11test/testcases/__init__.py
src/p11test/testcases/conftest.py      # shared fixtures for testcases
src/p11test/testcases/test_interface.py
src/p11test/testcases/test_slot.py
src/p11test/testcases/test_object.py
src/p11test/testcases/test_mechanism.py
src/p11test/testcases/test_encrypt.py
src/p11test/testcases/test_sign.py
...
CLAUDE.md, Python LSP, and all __init__.py files. After this chunk, `uv run python -c "import p11test"` works.
...
```bash
mkdir -p src/p11test/{cli,core,testcases}
mkdir -p tests
```

- [ ] **Step 2: Create src/p11test/__init__.py**

```python
"""p11test — CLI-first PKCS#11 test suite."""
...
```python
"""p11test configuration — TOML + CLI + env merge via pydantic-settings."""
...
```bash
uv run python -c "import p11test; print(p11test.__version__)"
```

Expected output: `0.1.0`

```bash
uv run python -c "from p11test.cli.app import main; print('CLI OK')"
```
...
Use the `mcp__plugin_serena_serena__activate_project` tool to activate the p11test project at `/home/user/src/m/p11test`.
...
Use `mcp__plugin_serena_serena__get_symbols_overview` on `src/p11test/cli/app.py` to verify symbols are detected (should show `app`, `version`, `main`).
...
class TestP11TestConfigToml:
    def test_toml_loading(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        toml_file = tmp_path / "p11test.toml"
...
```bash
git add src/p11test/config.py tests/test_config.py
...
- Modify: `src/p11test/core/loader.py`
...
Write `src/p11test/core/loader.py`:
...
```bash
git add src/p11test/core/loader.py tests/test_loader.py
...
- Modify: `src/p11test/core/isolation.py`
...
Write `src/p11test/core/isolation.py`:
...
```bash
git add src/p11test/core/isolation.py tests/test_isolation.py
...
**Files:**
- Modify: `src/p11test/cli/app.py`
- Create: `src/p11test/cli/test_cmd.py`
- Create: `src/p11test/cli/info_cmd.py`
- Create: `src/p11test/cli/list_cmd.py`
...
Write `src/p11test/cli/test_cmd.py`:
...
Write `src/p11test/cli/info_cmd.py`:
...
Write `src/p11test/cli/list_cmd.py`:
...
```bash
git add src/p11test/cli/ tests/test_cli.py
...
- Modify: `src/p11test/fixtures.py`
- Create: `src/p11test/testcases/conftest.py`
...
Update `src/p11test/fixtures.py`:
...
Add to `src/p11test/plugin.py` after the existing code:

```python
# Register fixtures from fixtures module
from p11test.fixtures import p11_config, p11_interface_version, p11_module, p11_session  # noqa: F401, E402
```
...
```bash
git add src/p11test/plugin.py src/p11test/fixtures.py src/p11test/testcases/conftest.py tests/test_plugin.py
...
These are the PRODUCT tests in `src/p11test/testcases/`.
...
export SOFTHSM2_CONF="${SOFTHSM2_CONF:-/tmp/p11test-softhsm2.conf}"
export TOKEN_DIR="${TOKEN_DIR:-/tmp/p11test-tokens}"
...
softhsm2-util --init-token --slot 0 --label "p11test" \
...
**Files:**
- Create: `src/p11test/testcases/conftest.py`
- Create: `src/p11test/testcases/test_interface.py`
- Create: `src/p11test/testcases/test_slot.py`
...
**Files:**
- Create: `src/p11test/testcases/test_encrypt.py`
- Create: `src/p11test/testcases/test_sign.py`
...
```bash
git add src/p11test/testcases/test_encrypt.py src/p11test/testcases/test_sign.py
