...
| last_action | Phase 5 CHECK-003 fixed via `ruff check --fix` (4 UP043 unnecessary `Generator[X, None, None]` → `Generator[X]` in fixtures.py + raw_fixtures.py). Full meta-test suite (`tests/`) is now green: 643 passed, 1 skipped, 0 failures (was 10 failures pre-Phase-5). |
