...
| last_action | Phase 5 CHECK-002 **CLOSED**. Final batch: types_std re-applied 3× (`__new__`, `__getnewargs__` w/ `# type: ignore[override]`, CKF dunders) via single-edit-commit pattern (file auto-reverts). pack.`element_type or ""`, extensions `mechanism_id: int \| None` annotation, recipes `init_token` per-byte list, `import_secret_key` widened, `read_attributes` already widened. test_x509/test_identity assert. `_classify_reason_code` `# type: ignore[return-value]`. test_selection `_record_sources` helper. CKR_ARGUMENTS_BAD added to `__all__`. test_mech_sign `pytest.skip` if param_recipe is None. `cli/test_cmd` wraps pin with SecretStr. **mypy: 0 errors. Cumulative CHECK-002: 516→0 (-516, -100%).** |
