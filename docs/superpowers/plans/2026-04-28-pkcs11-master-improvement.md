...
| last_action | Phase 2 aggregate-level triage. Populated findings table with provider+file-grouped clusters (most cluster patterns match documented MOD-BUGs from `docs/module-issues.md`). Two NEW findings need Phase 5: (1) tpm2 1,027 fixture-setup errors → CHECK-BUG (test fixtures should skip-on-unsupported-params, OR baseline should pre-skip the affected ACVP RSA / ECDSA / RSA-keygen vectors); (2) softhsm2-main 2 security/* file failures (test_arithmetic_overflow, test_ffi_length_boundary) → flagged for deeper investigation. Plus 1 doc update: opencryptoki pkcsslotd-die status to RESOLVED. |
