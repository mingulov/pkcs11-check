...
| GAP-P3 | Phase 4.5 (iter 37) | testcases/security/test_padding_oracle.py::TestAESPaddingOracle::test_cbc_pad_all_last_block_positions | MED | Vaudenay/POODLE: existing test_cbc_pad_error_uniformity probes only 2 positions (last byte vs middle of full ciphertext). Need all 16 positions of the LAST block, with enough trials to amortise the inherent ~6/256 probability that random corruption produces accidentally-valid PKCS#7 padding. | **CLOSED iter 48** — Added `test_cbc_pad_all_last_block_positions`: 20 trials × 16 byte positions = 320 corruption probes per run, statistical chance of false-pass ≈ 0.05%. Each trial uses an independent key + IV. Aggregates the resulting CKR set; if more than one distinct outcome is observed (typically `{CKR_OK, CKR_ENCRYPTED_DATA_INVALID}`), pytest.fail with Vaudenay 2002 / POODLE leak language and mitigation guidance (use AES-GCM or RFC 7366 encrypt-then-MAC). **Findings:** softhsm2-main FAILS — distinguishes CKR_OK from CKR_ENCRYPTED_DATA_INVALID at expected rates (Vaudenay channel exposed). kryoptic-main PASSES reliably across 20 trials — its CBC-PAD appears to defeat the leak channel (returns CKR_ENCRYPTED_DATA_INVALID even on accidentally-valid padding, or applies additional integrity beyond plain PKCS#7). Documented in `docs/module-issues.md` SoftHSM2 §"Vaudenay 2002 / POODLE channel on AES-CBC-PAD" as MEDIUM finding. mypy + ruff clean. |
| GAP-P4 | Phase 4.5 (iter 37) | testcases/security/test_padding_oracle.py::TestTimingBasic::test_aes_cbc_pad_decrypt_timing_sanity | MED | Lucky13-style timing for AES-CBC-PAD decrypt; only RSA decrypt timing previously covered (test_rsa_decrypt_timing_sanity). | **CLOSED iter 48** — Added `test_aes_cbc_pad_decrypt_timing_sanity`: parallel construction to the RSA timing test. Encrypts a known plaintext (105 bytes / 7 blocks) with AES-256-CBC-PAD; times 50 valid decrypts of the intact CT, then 50 invalid decrypts each with a different bit-flip in the last block. Compares averages; pytest.fail (CRITICAL with note + Lucky13 ref) if the ratio > 3x. Sanity-check level (not lab-grade timing analysis). Verified: softhsm2-main 1/1 PASS, kryoptic-main 1/1 PASS — neither shows >3x timing differential. mypy + ruff clean. |
