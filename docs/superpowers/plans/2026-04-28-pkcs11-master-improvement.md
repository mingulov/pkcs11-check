...
| last_action | TPM-FIXTURE-001 diagnosis revised + provisional fix: errors are TPM swtpm/tpm2-abrmd resource exhaustion (5,409× "out of memory for object contexts"), not fixture-can't-create-key. Raised tpm2-abrmd `--max-transient-objects=512 --max-sessions=512` (was default 27). Validation requires re-running tpm2 (~30-60 min). Closed OC-DOC-001 (added 2026-04-29 update note to module-issues.md OpenCryptoki section). Remaining queue: SOFTHSM-SEC-001 (MED), BOUNCY-CRASH-FREQ-001 (MED), TPM-FIXTURE-001 verification. |
