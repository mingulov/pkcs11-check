...
| GAP-T6 | Phase 4.5 (iter 37) | testcases/test_object_visibility.py::TestSessionObjectCrossVisibility | MED | "Cross-session object visibility (SoftHSM2 quirk) not asserted as security finding." | **CLOSED iter 47 (audit only)** — Gap was misframed. Per PKCS#11 v3.1 Sec.4.2, session objects ARE visible to all sessions of the same application by spec — they cannot be persisted but they are not per-session-scoped. The existing `test_session_object_visible_in_concurrent_session` already validates the spec-mandated behaviour AND emits a `compliance.note(VENDOR)` if the module over-isolates (some HSMs do, which is non-spec but more secure). The "SoftHSM2 quirk" framing in the gap analysis was a subagent miscount. The actual security boundary (session-object isolation across PROCESSES, not sessions) is a different concern with a different test scope. No new test added. |
