...
| phase1_last_run | **2026-04-30 DONE for 4/5 providers** — kryoptic / softhsm2 / nss-main / opencryptoki-master refreshed unfiltered. tpm2 FAILED to start (NEW infra issue TPM-DBUS-001 — see Fix Queue). Background task `bwueo1zox` exit 0. |
| phase1_due | false (just completed; tpm2 retry deferred until TPM-DBUS-001 resolved) |
...
| last_action | opencryptoki-master unfiltered DONE: 78,339 P / 2,588 F / 7,626 S / 55 XF / 88,608 T (vs filtered 75,209 P → **+2,588 real failures previously hidden**, +3,130 P, +7,615 legit skips). Combined unfiltered Phase 1 delta across 4/5 providers: **+3,085 real failures unmasked** (kryoptic +248, softhsm2 +123, nss +126, opencryptoki +2,588). tpm2 5/5 failed at dbus startup — new finding. |
...
| TPM-DBUS-001 | Phase 1 (iter 42) | docker/tpm2-pkcs11/run-tpm2.sh + Dockerfile | HIGH | tpm2 Phase 1 run failed at startup: `failed to allocate dbus proxy object: Error calling StartServiceByName for com.intel.tss2.Tabrmd: Cannot do system-bus activation with no user`. console.log truncated at this single error (4 lines total — script aborted before pytest started). dbus-daemon stderr is silenced (`2>/dev/null`), masking root cause. Apr 29 run with same Dockerfile worked; only intervening change is commit ff8cc65 (raised abrmd resource caps) which shouldn't affect dbus. Likely root causes: (a) dbus-daemon failed to start cleanly under fork, (b) /var/run/dbus/system_bus_socket missing/wrong ownership, (c) tabrmd .service file missing from /usr/share/dbus-1/system-services/. Fix plan: stop silencing dbus-daemon stderr; pre-check `/run/dbus/system_bus_socket` existence and `tabrmd.service` file before starting tabrmd; restart with explicit `--config-file=/usr/share/dbus-1/system.conf`; if needed switch to user-bus tcti (`tabrmd:bus_type=session`) which sidesteps system-bus auth. | **OPEN iter 42** — investigate inside the container interactively next iter (`docker run --rm -it docker-test-tpm2 bash`); reproduce the dbus init sequence step-by-step. Until fixed, tpm2 results stay frozen at 2026-04-29 baseline (8,272 P / 5,029 F / 1,027 errors / 59,225 T). |
