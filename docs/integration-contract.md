...
`report.jsonl` directly -- never by summing per-shard `quality.json` files (each shard's
`quality.json` is itself already a derived summary, and its counts may themselves be
lower bounds; re-deriving from the raw declared sources is what lets the merged block
tell an exact merged count from a merged lower bound instead of compounding one) -- so a
shard whose raw file is missing is a `missing_sources` entry, not a silently-absent
contribution. Older `quality.json` artifacts predating this
